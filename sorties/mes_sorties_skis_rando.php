...
{	
	...
	foreach(get_list_page($page,$nb_page) as $var){
		switch($var){
			...
			break;
			default:
			$liste_page .= '<a href="mes-sorties-skis-de-randonnees-p'.$var.'.html">&#8201;'.$var.'&#8201;</a> ';
		}
	}
	...
	if($nb_enregistrement > 0){
		...
		$data = get_file(TPL_ROOT.'sorties/mes_sorties_skis_rando.tpl');
		...
		$data = get_file(TPL_ROOT.'sorties/mes_sorties_skis_rando_empty.tpl');
		include(INC_ROOT.'header.php');
		$data = parse_var($data,array('liste_page'=>$liste_page,'nb_requetes'=>$db->requetes,'titre_page'=>'Liste des sorties - '.SITE_TITLE,'design'=>$_SESSION['design'],'ROOT'=>''));
	}
	echo $data;
