...
$sql = "SELECT * FROM point_gps LEFT JOIN refuge ON refuge.id_point = point_gps.id_point LEFT JOIN membres ON membres.id_m = point_gps.id_m WHERE point_gps.id_point = '$id_m'";
$result = $db->requete($sql);
$row2 = $db->fetch($result);

if (($row2['id_m'] == $_SESSION['mid'] AND $row['redacteur_points'] == 1) OR ($row['administrateur_points'] == 1)) 
{
	if($row['administrateur_points'] == 1)
	{
		$status = 2;
	}
	else
	...
	if($type == 'intro')
	{
		$db->requete("UPDATE refuge SET date_maj = NOW(), acces = '$text', acces_parse = '$text_parser' WHERE id_point = '$id_m'");
		$db->requete("UPDATE point_gps SET statut = '$status' WHERE id_point = '$id_m'");
	}
	elseif ($type == 'conclu')
	{
		$db->requete("UPDATE refuge SET date_maj = NOW(), remarques = '$text', remarque_parse = '$text_parser' WHERE id_point = '$id_m'");
		$db->requete("UPDATE point_gps SET statut = '$status' WHERE id_point = '$id_m'");
	}
	else
	{
