								...
								$up_description = $texte = $db->escape(zcode($_POST['textpage']));*/
									...
									/*	$db->requete("INSERT INTO mapgpx_pts (id_pts, lon, lat, ele, date, idmapgpx) 
										...
										if (empty($latb) and empty($longb))
										{
											...
											$lat1 = deg2rad(floatval($latb));
											$lon1 = deg2rad(floatval($longb));
											$lat2 = deg2rad(floatval($trkpt['lat']));
											$lon2 = deg2rad(floatval($trkpt['lon']));
											
											//echo $latb.'-'.$longb.'-'.$trkpt['lat'].'-'.$trkpt['lon'].'-'.$lat1.'-'.$lon1.'-'.$lat2.'-'.$lon2.'<br />';

											//echo $lon2;
											 
											// echo 'essai'.$lat1.'/'.$lat2.'<br />';
											//Formule simple
									...
									$sql2 = "INSERT INTO map_gpx VALUES('', NOW(), '$titre', '$url_mapgpx', '$texte','$texte_parser', '$cle',
									'$nbpoint', '$longueur', '$den_positif_cumu', '$den_negatif_cumu', '$altitude_maxi', '$altitude_mini',
									'$altitude_moyenne', '$date_debut', '$date_fin', '$duree_totale', '$duree_pause', '$duree_marche', '$vitesse_moyenne',
									'$duree_positif_cumu', '$duree_negatif_cumu','$vitesse_montee', '$vitesse_descente', '0', '$_SESSION[mid]', '$activite', '$idt', '0')";
									$db->requete($sql2);
									
									$id_trace = $db->last_id();
									
									if(isset($_SESSION['tmp_Img'])){
										$sql = "UPDATE images SET tmp = '0', s_dir = '".$id_trace."' WHERE dir = '9' AND s_dir = '0' AND tmp = '1'";
										$db->requete($sql);
		...
		if($row['supprimer_trace'] == 1 OR $result['id_m'] == $_SESSION['mid'])
		{
			...
			else
			{
				if(isset($_POST['valider']) AND $_POST['valider'] == 1)
				{
					...
					$text_parser = "Votre trace GPX ".$result['nom_mapgpx']." viens d\'être supprimée de notre serveur.";
					$text = "Votre trace GPX ".$result['nom_mapgpx']." viens d\'être supprimée de notre serveur.";
