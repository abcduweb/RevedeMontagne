...
if (isset ($_SESSION['ses_id'])) {
	if(!empty($_GET['pid'])){
		...
		if (isset ($_POST['texte']) AND !empty ($_POST['texte']) AND strlen(trim($_POST['texte'])) > 2) {
			$sql = "SELECT nom_som, com_point.id_m AS id_m, date_ajout, UNIX_TIMESTAMP(date_ajout) FROM sommets 
					LEFT JOIN com_point ON com_point.id_point = sommets.id_point 
					WHERE sommets.id_point = '$pid' ORDER BY date_ajout DESC";
			$db->requete($sql);
			$row = $db->fetch_assoc();
			if($db->num() > 0){
				...
				if($right[0] == 1){
					...
					if($row['id_m'] != $_SESSION['mid'] OR ($row['id_m'] == $_SESSION['mid'] AND $date_limit < time())){
						...
						$redirection = ROOT.'detail-'.title2url($row['nom_som']).'-'.$pid.'.html#r'.$msg_id;
					...
					$redirection = ROOT.'photo-'.title2url($row['nom_refuge']).'-commentaires-'.$pid.'.html';
