...
else
{
	...
	if($auth['redacteur_topo'] == 1 OR $auth['administrateur_topo'] == 1)
	{
		...
		if($_SESSION['mid'] == $donnees['id_m'] OR $auth['administrateur_topo'] == 1)
		{
			...
			$data = get_file(TPL_ROOT.'topos/edit_fiche_topo_ski.tpl');
			...
			//Gestion de la difficultés "ski"
			$diff_topo_ski2='';
			$selected='';
			foreach($diff_topo_ski as $cle_diff_topo_ski => $diff_ski2)
			{
				if($diff_ski2 == $donnees['difficulte'])
					$selected='selected="selected"';
				else
					$selected='';
				$diff_topo_ski2.='<option value="'.$diff_ski2.'" '.$selected.'>'.$diff_ski2.'</option>';
			}
			
			//Gestion de l'exposition
			$expo2='';
			$selected='';
			foreach($expo as $cle_expo => $expos2)
			{
				if($expos2 == $donnees['exposition'])
					$selected='selected="selected"';
				else
					$selected='';
				$expo2.='<option value="'.$expos2.'" '.$selected.'>'.$expos2.'</option>';
			}
			...
			$data = parse_var($data,array('id_topo'=>$topo, 'massif'=>$donnees['nom_massif'], 'sommet'=>$donnees['nom_point'],
										...
										'orientation'=>$orientation, 'diff_topo_monte'=>$diff_topo_monte2, 'diff_ski'=> $diff_topo_ski2, 'expo'=>$expo2,
										'nb_jours'=>$nb_jours2, 'depart'=>$depart, 'type_iti'=>$type_iti, 'carte'=>$carte_edite, 'pente'=>$donnees['pente'],
		...
		else
		{
			...
			$redirection = 'liste-des-topos-skis-rando.html';
			echo display_notice($message,'important',$redirection);
		}
