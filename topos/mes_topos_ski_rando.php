...
{	
	...
	$sql = ("SELECT * FROM topos
				...
				WHERE topos.id_activite = '1' AND topos.id_m = '".$_SESSION['mid']."' and topos.visible < 3");	
	$db->requete($sql);
	//$nb_pages = $db->num();
	...
	if($nb_enregistrement > 0){
		...
		$data = get_file(TPL_ROOT.'topos/my_topos_skis.tpl');
		...
		$sql = "SELECT visible, topos.id_m, topos.id_topo, nom_topo, topos.id_activite, nom_point, nom_topo, nom_activite,  nom_massif, denniveles   FROM topos
				...
				LEFT JOIN departs ON departs.id_depart = topos.id_depart
				LEFT JOIN activites ON activites.id_activite = topos.id_activite
				WHERE topos.id_activite = '1' AND topos.id_m = '".$_SESSION['mid']."'
				AND topos.visible < 3";
		...
		while ($row = $db->fetch($result))
		{
			...
			{
				$modif = '<a href="edition-fiche-topo-ski-'.$row['id_topo'].'.html"><img src="'.$config['domaine'].'/templates/images/1/form/edit.png" alt="Editer"></a>';
				$supprimer = '<a href="'.$config['domaine'].'topos/envoi_topo_skis.php?idt='.$row['id_topo'].'&del=1"><img src="'.$config['domaine'].'/templates/images/1/form/supprimer.png" alt="Supprimer"></a>';
			}
			else
			{
				$modif = '';
			...
			if ($row['visible'] == 1)
				...
				$valider = '<a href="'.$config['domaine'].'topos/envoi_topo_skis.php?idt='.$row['id_topo'].'&val=1"><img src="'.$config['domaine'].'/templates/images/1/form/faire_valider.png" alt="Demander une validation"></a>';
		...
		foreach(get_list_page($page,$nb_page) as $var){
			switch($var){
				case $page:
					$liste_page .= '<span class="current">&#8201;'.$var.'&#8201;</span> ' ;
				break;
				case '<a href="mes-topos-skis-rando.html">&#8201;...&#8201;</a> ':
					$liste_page .= $var;
				break;
				default:
					$liste_page .= '<a href="mes-topos-skis-rando.html?type='.$current_type.'&amp;order='.$current_order.$search.'&amp;page='.$var.'">&#8201;'.$var.'&#8201;</a>';
			}
		}
		...
		$data = get_file(TPL_ROOT.'topos/my_topos_skis_empty.tpl');
		include(INC_ROOT.'header.php');
		$data = parse_var($data,array('liste_page'=>$liste_page,'nb_requetes'=>$db->requetes,'titre_page'=>'Liste des topos - '.SITE_TITLE,'design'=>$_SESSION['design'],'ROOT'=>''));
	}
	echo $data;
