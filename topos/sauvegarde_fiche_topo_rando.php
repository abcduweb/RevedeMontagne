...
if(!empty($_GET['pid']))
{
	$midi = intval($_GET['pid']);
		$reponse =  $db->requete("SELECT id_m_join, invisible, topos.id_topo, nom_topo, idf, id_activite, nom_point, id_point, nom_topo, topos.id_massif, nom_massif, orientation, denniveles, difficulte_topo, difficulte, topos.statut, exposition,
								   lieu_depart, alt_depart, pente, nb_jours, nom_type, acces_parser, itineraire_parser, remarque_parser, altitude, topos.id_m, membres.pseudo, avatar
									FROM topos
	...
	if($num > 0){
		$data = get_file(TPL_ROOT."topos/topo_randonnee/fiche_topos_rando.tpl");
	...
	//  	
		...
		$data = parse_var($data,array('fichierGPX'=>$mapgpx, 'ajout_sortie'=>$ajout_sortie,'ajout_trace'=>$ajout_trace, /*'repondre'=>$reponse,*/ 'design'=>$_SESSION['design'], 'idt'=>$midi,  'mapid'=>'zoom_canvas'));
	}else{
