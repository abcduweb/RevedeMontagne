...
else
{
	...
	if($auth['redacteur_topo'] == 1 OR $auth['administrateur_topo'] == 1)
	{
		...
		if(isset($_GET['del']) AND $_GET['del'] == 1)
		{
			...
			if($num == 1 AND $donnees['id_m'] == $_SESSION['mid'] OR $auth['administrateur_topo'] == 1)
			{
				if(isset($_GET['valider']) AND $_GET['valider'] == 1)
				{
					...
					$type = "ok";
					$redirection = DOMAINE.'mes-topos-skis-rando.html';
				...
				{
					$url = 'envoi_topo_skis.php?idt='.$idt.'&amp;del='.$del.'&amp;valider=1';
			...
			else
			{
				...
				$type = "important";
				$redirection = DOMAINE.'mes-topos-skis-rando.html.html';
				$data = display_notice($message,$type,$redirection);
			}
		...
		elseif(isset($_GET['del']) AND $_GET['del'] == 1)
		{
			...
			$redirection = ROOT."mes-topos-skis-rando.html.html";
		...
		elseif(isset($_GET['val']) AND $_GET['val'] == 1)
		{
			...
			if($num == 1 AND $donnees['id_m'] == $_SESSION['mid'] OR $auth['administrateur_topo'] == 1)
			{
				if(isset($_GET['valider']) AND $_GET['valider'] == 1)
				{
					...
					$text_parser = 'Un nouveau topo de skis randonn&eacute;e vient d\&ecirc;tre mis en ligne<br /> 
									Vous pouvez le consulter ici : <a href="topo-'.title2url($donnees['nom_topo']).'-t'.$donnees['id_topo'].'.html">'.$nomtopo.'</a><br /><br />
									-----------------<br />
					...
					$sql = "SELECT * FROM autorisation_globale LEFT JOIN membres ON membres.id_group = autorisation_globale.id_group WHERE autorisation_globale.administrateur_topo = 1 LIMIT $limite,5";
					$validateur = $db->requete($sql);
					$sql = "INSERT INTO liste_discutions VALUES('','<strong>[Topo skis]</strong> ".$nomtopo."','',$idM,$_SESSION[mid],0,0,0)";
					$db->requete($sql);
					$idDiscu = $db->last_id();
					$nb_participant = 0;
					while($to = $db->fetch($validateur)){
				...
				{
					$url = 'envoi_topo_skis.php?idt='.$idt.'&amp;val='.$val.'&amp;valider=1';
		...
		else
		{
			...
			if( (isset($_POST['nom_topo']) AND strlen(trim($_POST['nom_topo'])) > 2) 
				...
				AND (isset($_POST['diff_ski']) AND strlen(trim($_POST['diff_ski'])) > 0) 
				AND (isset($_POST['expo']) AND strlen(trim($_POST['expo'])) > 0) 
				AND (isset($_POST['carte'])) 
				AND (isset($_POST['depart']) AND strlen(trim($_POST['depart'])) > 0) 
				AND (isset($_POST['pente']) AND strlen(trim($_POST['pente'])) > 0) 
			  ...
			  )
			{
				...
				$diff_ski    = htmlentities($_POST['diff_ski'],ENT_QUOTES, 'UTF-8');
				$expo        = intval($_POST['expo']);
				$carte       = $_POST['carte'];
				$depart      = intval($_POST['depart']);
				$pente       = htmlentities($_POST['pente'],ENT_QUOTES, 'UTF-8');
				...
				$_SESSION['diff_ski'] = $diff_ski ;   
				$_SESSION['expo'] = $expo;        
				$_SESSION['carte'] = $carte;       
				$_SESSION['depart'] = $depart;      
				$_SESSION['pente'] = $pente;      
				...
				{		
					...
					//Insertion des donn&eacute;es dans la table topo skis
					$sql = "INSERT INTO topos_skis VALUES
							(
							'',
							'$topo',
							'$diff_ski',
				...
				else
				{
					...
					$reponse =  $db->requete("SELECT nom_massif, nom_point, 
												nom_topo, denniveles, topos.id_m AS id_m, pente, id_sommet,
												orientation, difficulte_topo, difficulte, exposition, nb_jours, departs.id_depart AS id_depart, type_topo.id_type_iti AS id_type_iti,
												itineraire, remarque
												FROM topos
												LEFT JOIN topos_skis ON topos_skis.id_topo = topos.id_topo
												LEFT JOIN point_gps ON point_gps.id_point = topos.id_sommet
					...
					if($num == 1 AND $auth['redacteur_topo'] == 1 OR $auth['administrateur_topo'] == 1)
					{
						...
						//On update les donn&eacute;es de la table topos_skis
						$sql = "UPDATE topos_skis 
						SET difficulte = '$diff_ski',
							pente = '$pente',
							exposition = '$expo'
						WHERE id_topo = '".$e."'";
						$db->requete($sql);
					...
					else
					{
						...
						$type = "important";
						$redirection = 'mes-topos-skis-rando.html';
						$data = display_notice($message,$type,$redirection);
					}
