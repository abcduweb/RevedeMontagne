...
if(!empty($_GET['pid']))
{
	...
	if($num > 0 && ($donnees['visible'] == 1) || $num > 0 && ($donnees['id_m'] == $_SESSION['mid'])){
		...
		$data = parse_var($data,array(
			...
			'edit_rapide_intro' => 'ondblclick="inlineMod('.$midi.', this, \'itineraire\');"',
			'edit_rapide_conclu' => 'ondblclick="inlineMod('.$midi.', this, \'remarques\');"',
			));	
	...
	//  
		//On vérifie s'il y a des images dans le topo et si oui on en choisi une en RAND.
		$reponse2 =  $db->requete("SELECT id_image, nom FROM images WHERE dir = 8 AND s_dir='".$midi."' ORDER BY RAND(id_image) LIMIT 1");
		$donnees2 = $db->fetch($reponse2);
		
		$config['og_facebook_image'] = $donnees2['nom'];
