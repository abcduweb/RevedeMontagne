...
##########################################
	...
	if($nb_enregistrement > 0){
		...
		$data = get_file(TPL_ROOT.'l_topo_rando.tpl');
		...
		foreach(get_list_page($page,$nb_page) as $var){
			switch($var){
				...
				break;
				case '<a href="liste-des-topos-de-randonnee.html">&#8201;...&#8201;</a> ':
					$liste_page .= $var;
				break;
				default:
					$liste_page .= '<a href="liste-des-topos-de-randonnee.html?type='.$current_type.'&amp;order='.$current_order.'{search}&amp;page='.$var.'">&#8201;'.$var.'&#8201;</a>';
			}
		}
		...
		$data = get_file(TPL_ROOT.'l_topo_rando_vide.tpl');
		include(INC_ROOT.'header.php');
