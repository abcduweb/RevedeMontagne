...
function inlineMod(id, obj, type)
{
	if(editionEnCours)
	{
		return false;
	...
	XHR.open("POST", "ajax/topos/randos/load_edit.php", true);
	XHR.onreadystatechange = function()
	{
		...
		if (XHR.readyState == 4)
		{
			...
			boutonSauver.onclick = function sortir(){
				sauverMod(id, obj, input.value, type);
	...
	var var_transmettre = "id="+id+"&type="+type;
...
function sauverMod(id, obj, valeur, type)
{
	...
	XHR.open("POST", "ajax/topos/randos/save_edit.php", true);
	XHR.onreadystatechange = function()
	{
	...
	var var_transmettre = "m="+id+"&type="+type+"&texte="+valeur;
	XHR.send(var_transmettre);	
}